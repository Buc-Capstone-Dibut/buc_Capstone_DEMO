import { AnalysisResult, JobData } from "@/store/interview-setup-store";
...
interface MockInterviewSessionMeta {
  company?: string;
  role?: string;
  mode?: string;
...
export function buildMockInterviewReportModel({
  analysis,
  jobData,
  session,
}: {
  ...
  session?: MockInterviewSessionMeta;
}): MockInterviewReportModel {
  const company = session?.company || jobData?.company || "모의면접";
  const role = session?.role || jobData?.role || "직무 정보 없음";
  ...
  const primaryImprovement = analysis.feedback.improvements[0] || "답변 첫 문장의 밀도를 높여보세요.";
  const primaryStrength = analysis.feedback.strengths[0] || "직무 이해도와 답변 구조가 안정적입니다.";

  return {
    badgeLabel: "이번 면접의 디벗 유형",
    typeName,
    typeLabels,
    summary: `${primaryStrength} 다만 ${primaryImprovement} 흐름을 보완하면 전체 인상이 더 강해집니다.`,
    heroMetrics: [],
    metaItems: [
      { label: "직무", value: role },
      originalUrl
    ...
    strengths: (analysis.feedback.strengths.length > 0 ? analysis.feedback.strengths : ["강점 데이터가 아직 없습니다."]).slice(0, 3),
    weaknesses: (analysis.feedback.improvements.length > 0 ? analysis.feedback.improvements : ["개선 포인트 데이터가 아직 없습니다."]).slice(0, 3),
    ...
    fitSummary: `${company}의 ${role} 면접 기준으로 보면, 답변의 구조와 직무 연결성은 좋았고 전달 밀도를 조금만 끌어올리면 더 강한 인상을 줄 수 있습니다.`,
