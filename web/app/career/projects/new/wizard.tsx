...
import {
  getWorkspaceProjectImportCandidatesAction,
  markWorkspaceProjectCandidateImportedAction,
  prepareWorkspaceProjectDraftAction,
  saveProjectAction,
} from "../actions";
import type {
  ProjectInput,
...
const STEPS = [
  ...
  {
    id: "s",
    question: "이 프로젝트는 어떤 배경에서 시작하게 된 건가요?",
    placeholder:
  ...
  {
    id: "i",
    question: "이 프로젝트를 통해 새롭게 깨닫거나 배운 점은 무엇인가요?",
    placeholder:
...
const IMPORT_LOADING_STEPS = [
  ...
  {
    id: "analyze",
    title: "프로젝트 내용을 분석하는 중",
    description: "핵심 성과와 프로젝트 포인트를 추출하고 있어요.",
  },
  {
    id: "draft",
    title: "프로젝트 초안을 작성하는 중",
...
function buildCandidatePeriod(candidate: WorkspaceProjectImportCandidate) {
...
export default function ProjectWizardClient() {
  ...
  const [candidates, setCandidates] = useState<WorkspaceProjectImportCandidate[]>(
    [],
  );
  ...
  useEffect(() => {
    ...
    const loadCandidates = async () => {
      try {
        ...
        const next = await getWorkspaceProjectImportCandidatesAction();
        if (!cancelled) {
          setCandidates(next);
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    if (!formData.company?.trim()) {
      alert("프로젝트명(제목)은 필수입니다. 작성 항목으로 돌아갑니다.");
    ...
    setIsSaving(true);
    try {
      const saved = await saveProjectAction(formData as ProjectInput);
      ...
      if (
        ...
        try {
          await markWorkspaceProjectCandidateImportedAction(
            selectedWorkspaceId,
            saved.experience.id,
          );
        } catch (markError) {
  ...
  const handleApplyWorkspaceCandidate = async (
    ...
    try {
      ...
      const result = await prepareWorkspaceProjectDraftAction(workspaceId);
      if (!result.success) {
        throw new Error("워크스페이스 프로젝트 초안 불러오기에 실패했습니다.");
      }

      const nextTags = Array.isArray(result.draft.tags) ? result.draft.tags : [];
      const nextTechStack = Array.isArray(result.draft.techStack)
        ? result.draft.techStack
        : [];
      setFormData((prev) => ({
        ...prev,
        ...result.draft,
      ...
      toast.success(`'${result.workspaceName}' 프로젝트 초안을 불러왔습니다.`);
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : "워크스페이스 프로젝트 초안 불러오기에 실패했습니다.",
      );
    } finally {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="h-16 flex items-center justify-between px-6 border-b border-slate-100 dark:border-slate-800">
        <div className="font-bold text-[14px]">나의 프로젝트 기록하기</div>
        <button
          onClick={() => router.push("/career/projects")}
            ...
            <div className="space-y-8">
              ...
              <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
                ...
                <p className="mt-2 text-xs text-slate-600 dark:text-slate-300">
                  후보를 선택하면 AI가 워크스페이스 프로젝트 내역을 분석해 기본정보와
                  질문 단계 초안을 자동으로 채웁니다.
                </p>
            ...
            <div className="space-y-10" key={currentStepIndex}>
              <div className="space-y-4">
                ...
                {workspaceDraftNotice && selectedWorkspaceName && currentStepIndex === 1 ? (
                  <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-700">
                    {selectedWorkspaceName} 프로젝트 분석 초안이 반영되어 있습니다. 필요하면
                    수정하고 진행하세요.
                  </div>
                ) : null}
              </div>
      ...
      <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>워크스페이스 프로젝트 불러오기</DialogTitle>
            <DialogDescription>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 px-4 backdrop-blur-[2px]">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-background p-5 shadow-2xl dark:border-slate-800">
            ...
            <p className="mt-4 text-[11px] text-muted-foreground">
              AI가 워크스페이스 프로젝트를 분석해 작성 초안을 만드는 중입니다.
            </p>
          </div>
        </div>
      ) : null}
