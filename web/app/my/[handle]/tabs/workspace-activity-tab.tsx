...
function joinedDurationLabel(joinedAt: string | null): string {
  if (!joinedAt) return "정보 없음";
  const joined = new Date(joinedAt);
  if (Number.isNaN(joined.getTime())) return "정보 없음";
  const diffMs = Math.max(Date.now() - joined.getTime(), 0);
          ...
          <div className="px-4 py-3 border-b bg-muted/30 flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-muted-foreground truncate">
                ...
                {workspace.joinedAt
                  ? new Date(workspace.joinedAt).toLocaleDateString("ko-KR")
                  : "가입일 미상"}
              </p>
            </div>
          ...
          <div className="p-4 space-y-3">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              ...
              <div className="rounded-lg border p-3 space-y-1">
                ...
                <p className="text-lg font-semibold tabular-nums">
                  {joinedDurationLabel(workspace.joinedAt)}
                </p>
              </div>
            </div>
