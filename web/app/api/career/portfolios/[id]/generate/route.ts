...
async function generatePortfolioFreeSlides(input: {
  ...
  const templateGuide =
    input.templateId === "developer-minimal"
      ? `[Minimal Tech]
- 톤: 흰 배경(bg-white), 넓은 여백, 깔끔한 sans-serif
- 색: 거의 흑백 + 한 가지 강조색만(emerald/sky/blue)
- 디자인: 큰 타이포(text-6xl+), 좌측 정렬, 단순한 그리드, 얇은 border
- 슬라이드 7~9장`
      : input.templateId === "case-study"
        ? `[Editorial Magazine]
- 톤: 크림/베이지 종이 톤(bg-amber-50, bg-stone-50)
- 색: warm earth + 인쇄물 느낌의 brown(amber-700, stone-800)
- 디자인: serif 헤딩(font-serif), 매거진 컬럼, 큰 인용구(blockquote), drop cap
- 슬라이드 11~13장`
        : `[Bold Showcase]
- 톤: 어두운 배경(bg-slate-900, bg-zinc-950), 강한 컬러 블록
- 색: 다크 + 비비드 강조색(pink-500, cyan-400, amber-400)
- 디자인: 거대한 BigNumber(text-8xl+), 컬러 블록, 텍스트 흰색, 강한 대비
- 슬라이드 9~11장`;

  const seed = Math.random().toString(36).slice(2, 10);
  const prompt = `너는 시니어 UI 디자이너다. 사용자의 포트폴리오 데이터를 받아서 *발표용 16:9 슬라이드* 를 HTML+Tailwind 로 직접 디자인한다.

[★ 핵심 — 자유롭게 디자인하되 안전 규칙만 지킨다]
각 슬라이드는 \`<div class="h-full w-full ...">...</div>\` 형태의 *완성된 HTML 한 덩어리*다.
ChatGPT 가 index.html 을 작성하듯 자유롭게 시각 디자인하고, Tailwind 유틸리티 클래스를 풍부하게 사용해라.

[안전 규칙 — 어기면 자동 제거됨]
1. 사용 가능한 태그: div, section, header, footer, article, main, aside, p, h1~h6, span, strong, em, ul, ol, li, blockquote, figure, figcaption, hr, br
2. 금지 태그: script, iframe, style, link, img, a, button, input, form, video, audio, svg
3. 금지 className:
   - position: absolute, fixed, sticky (요소 겹침 방지)
   - z-*, inset-*, top-/left-/right-/bottom-* (단 0 은 허용)
   - 음수 margin (-mt-*, -mb-* 등)
   - 큰 고정 사이즈 (w-[1000px], h-[2000px], h-[300vh] 등)
4. 텍스트는 반드시 HTML escape 된 평문. on*= 같은 이벤트 핸들러 금지.
5. 슬라이드 루트는 반드시 \`h-full w-full overflow-hidden\` 포함.

[디자인 자유도 — *적극적으로* 풍부한 디자인 만들어라]
- Tailwind grid (grid grid-cols-2, grid grid-cols-[1fr_2fr], grid grid-rows-2 등) 자유 사용
- flex (flex flex-col, flex items-center, justify-between 등) 자유 사용
- 큰 타이포 적극 — h1 은 보통 text-5xl ~ text-7xl. 메트릭/숫자는 text-7xl ~ text-9xl
- 강조 도구 풍부하게:
  * 좌측 막대 border-l-[6px] border-emerald-500 pl-8
  * 상단 막대 border-t-[4px] border-current pt-4
  * 컬러 블록 배경 (bg-emerald-100, bg-amber-100, bg-slate-800)
  * 라운드 카드 (rounded-2xl bg-white shadow-lg p-8)
  * 번호 원 (h-12 w-12 rounded-full bg-emerald-500 text-white flex items-center justify-center)
  * 인용 부호 (text-7xl text-amber-700/30 → before/after 대신 그냥 span 으로 큰 따옴표)
  * 그라데이션 (bg-gradient-to-br from-slate-900 to-slate-700)
  * tracking (tracking-[0.2em], tracking-tight)
  * line-height (leading-[0.9], leading-7)
- 컬러 톤은 템플릿 가이드 따라

[콘텐츠 자유도]
- 사용자 데이터의 description/situation/role/solution/result/lesson/techStack 을 *그대로 인용*. 없는 수치·회사명·성과 만들지 마라.
- 한 슬라이드에 한 가지 메시지. 너무 많은 텍스트 금지(가독성).
- 각 슬라이드는 시각적으로 *완전히 다른* 디자인. 같은 레이아웃 반복 금지.

[슬라이드 수와 흐름]
${templateGuide}
- 흐름: 표지(임팩트) → 프로필 1장 → 기술 스택 1장 → 프로젝트별 2~3장 → 회고/마감 1~2장

[랜덤 시드: ${seed}]
이 시드를 기준으로 매번 다른 색·구도·강조 방식을 선택해라.
...
[반환 형식 — JSON 하나만, 코드 펜스 안에]
{
  "slides": [
    {
      "id": "slide-1",
      "intent": "표지 — 백엔드 리드의 강점 한 줄",
      "html": "<div class=\\"h-full w-full overflow-hidden grid grid-cols-[1.2fr_1fr] gap-12 px-20 py-16 bg-white\\"><div class=\\"flex flex-col justify-center\\"><p class=\\"text-xs font-black uppercase tracking-[0.3em] text-emerald-600\\">PORTFOLIO 2026</p><h1 class=\\"mt-4 text-6xl font-black leading-[0.95] text-slate-950\\">백엔드 리드 김지원</h1><p class=\\"mt-6 max-w-[460px] text-base font-bold leading-7 text-slate-700\\">1만 동시 접속에서도 P99 220ms 를 지키는 시스템을 설계합니다.</p></div><div class=\\"flex flex-col justify-center border-l-[8px] border-emerald-500 pl-10\\"><p class=\\"text-7xl font-black text-emerald-600 leading-none\\">850→220ms</p><p class=\\"mt-3 text-xs font-bold uppercase tracking-[0.2em] text-slate-500\\">P99 응답시간</p><p class=\\"mt-8 text-sm font-bold text-slate-600\\">동시 접속 1만 명 안정 운영</p></div></div>"
    },
    {
      "id": "slide-2",
      "intent": "Aurora 케이스 — 문제부터 결과까지 한 호흡",
      "html": "<div class=\\"h-full w-full overflow-hidden flex flex-col gap-8 px-16 py-12 bg-white\\"><div class=\\"flex items-end justify-between\\"><div><p class=\\"text-xs font-black uppercase tracking-[0.3em] text-emerald-600\\">CASE 1 · AURORA COMMERCE</p><h2 class=\\"mt-3 text-4xl font-black leading-tight text-slate-950\\">동시 접속 1만 명에서<br/>채팅이 끊기지 않게</h2></div><div class=\\"text-right\\"><p class=\\"text-5xl font-black text-emerald-600 leading-none\\">7개월</p><p class=\\"mt-2 text-xs font-bold uppercase tracking-wider text-slate-500\\">마이그레이션 책임</p></div></div><div class=\\"grid grid-cols-4 gap-6 flex-1\\"><div class=\\"flex flex-col gap-2 border-t-[4px] border-slate-200 pt-4\\"><span class=\\"flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-sm font-black text-slate-600\\">01</span><p class=\\"mt-2 text-xs font-bold uppercase tracking-wider text-slate-400\\">문제</p><p class=\\"text-sm font-bold leading-6 text-slate-700\\">트래픽 5천 명 한계, 시즌마다 메시지 끊김</p></div><div class=\\"flex flex-col gap-2 border-t-[4px] border-slate-300 pt-4\\"><span class=\\"flex h-10 w-10 items-center justify-center rounded-full bg-slate-200 text-sm font-black text-slate-700\\">02</span><p class=\\"mt-2 text-xs font-bold uppercase tracking-wider text-slate-400\\">역할</p><p class=\\"text-sm font-bold leading-6 text-slate-700\\">백엔드 리드 합류, 아키텍처 재설계 책임</p></div><div class=\\"flex flex-col gap-2 border-t-[4px] border-emerald-300 pt-4\\"><span class=\\"flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100 text-sm font-black text-emerald-700\\">03</span><p class=\\"mt-2 text-xs font-bold uppercase tracking-wider text-slate-400\\">해결</p><p class=\\"text-sm font-bold leading-6 text-slate-700\\">Redis Pub/Sub + Kafka 비동기 분리</p></div><div class=\\"flex flex-col gap-2 border-t-[4px] border-emerald-500 pt-4\\"><span class=\\"flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500 text-sm font-black text-white\\">04</span><p class=\\"mt-2 text-xs font-bold uppercase tracking-wider text-emerald-600\\">결과</p><p class=\\"text-sm font-bold leading-6 text-emerald-800\\">P99 220ms · 다운타임 0건 유지</p></div></div></div>"
    }
  ]
}

총 슬라이드 수는 위 템플릿 가이드 분량 따른다. 표지 → 프로필 1장 → 기술 스택 1장 → 프로젝트 2~3장씩 → 회고/마감 흐름. *각 슬라이드는 시각적으로 완전히 다른 디자인*.`;

  try {
    const genAI = new GoogleGenerativeAI(apiKey);
    // Gemini 2.5 Pro 사용 — Flash 보다 디자인 직관과 출력 풍성도가 훨씬 좋음.
    // 대신 응답 시간이 길어질 수 있으므로 SSE heartbeat 가 살아있어야 함.
    const model = genAI.getGenerativeModel({
