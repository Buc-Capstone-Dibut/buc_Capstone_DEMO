  ...
  return (
    <svg viewBox="0 0 800 450" className="w-full h-full font-sans select-none" style={{ backgroundColor: "hsl(var(--background))" }}>
      ...
      <g transform="translate(100, 100)">
              ...
              return (
                <g key={`col-${c}`} transform={`translate(${c * 50}, ${r * 50})`}>
                  <rect width="45" height="45" rx="8" fill={highlighted ? "url(#primary-grad)" : "url(#surface-grad)"}
                        stroke={highlighted ? "hsl(0 0% 100%)" : "hsl(var(--border))"} strokeWidth="2" filter={highlighted ? "url(#soft-shadow)" : ""} />
                  <text x="22.5" y="27" textAnchor="middle" fontSize="14" fontWeight="800" fill={highlighted ? "hsl(0 0% 100%)" : "hsl(var(--muted-foreground))"}>{r},{c}</text>
                </g>
              );
            })}
          </g>
        ...
        {/* Helper vectors */}
        <text x="-15" y="77" textAnchor="end" fontSize="16" fontWeight="900" fill="hsl(239 84% 67%)">Row: 1</text>
        <path d="M 122.5 -5 L 122.5 -35" stroke="hsl(160 84% 39%)" strokeWidth="3" markerEnd="url(#arrow-head)" opacity="0.5" />
        <text x="122.5" y="-45" textAnchor="middle" fontSize="16" fontWeight="900" fill="hsl(160 84% 39%)">Col: 2</text>
      </g>

      <path d="M 330 170 L 400 170" stroke="url(#primary-grad)" strokeWidth="4" markerEnd="url(#arrow-head)" strokeDasharray="8 6" />
      <text x="365" y="150" textAnchor="middle" fontSize="14" fontWeight="700" fill="hsl(239 84% 67%)">매핑</text>
          ...
          return (
            <g key={`flat-${i}`} transform={`translate(${15 + (i%4) * 60}, ${20 + Math.floor(i/4) * 60})`}>
              <rect width="50" height="50" rx="8" fill={highlighted ? "url(#primary-grad)" : "hsl(var(--muted))"}
                    stroke={highlighted ? "hsl(0 0% 100%)" : "hsl(var(--border))"} strokeWidth="1" />
              <text x="25" y="32" textAnchor="middle" fontSize="18" fontWeight="800" fill={highlighted ? "hsl(0 0% 100%)" : "hsl(var(--foreground))"}>{i}</text>
            </g>
          );
        })}
        {/* Ping animation to show map connection */}
        <motion.circle cx="160" cy="105" r="28" fill="none" stroke="hsl(271 91% 65%)" strokeWidth="3" filter="url(#glow)"
      ...
      <text x="400" y="380" textAnchor="middle" fontSize="16" fontWeight="600" fill="hsl(var(--foreground))">
        1D 인덱스 = (<tspan fill="hsl(239 84% 67%)" fontWeight="800">r</tspan> × 열의 개수) + <tspan fill="hsl(160 84% 39%)" fontWeight="800">c</tspan>
      </text>
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 800 450" className="w-full h-full font-sans select-none" style={{ backgroundColor: "hsl(var(--background))" }}>
      ...
      <g transform="translate(100, 160)" filter="url(#soft-shadow)">
        ...
        <text x="25" y="40" fontSize="14" fontFamily="monospace" fontWeight="800" fill="hsl(239 84% 67%)">for (r = 0; r &lt; 3; r++) {"{"}</text>
        <text x="45" y="70" fontSize="14" fontFamily="monospace" fontWeight="800" fill="hsl(160 84% 39%)">for (c = 0; c &lt; 4; c++) {"{"}</text>
      ...
      <g transform="translate(420, 120)">
        ...
        <motion.rect width="50" height="50" rx="8" fill="url(#primary-grad)" stroke="hsl(0 0% 100%)" strokeWidth="2" filter="url(#glow)"
           animate={{
        ...
        {/* Tracking texts */}
        <motion.path d="M 235 25 L 260 25" stroke="hsl(239 84% 67%)" strokeWidth="3" markerEnd="url(#arrow-head)"
           animate={{ y: [0, 0, 0, 0, 60, 60, 60, 60, 120, 120, 120, 120] }} transition={{ duration: 6, ease: "linear", repeat: Infinity }} />
        <text x="270" y="30" fontSize="14" fontWeight="800" fill="hsl(239 84% 67%)">Outer r</text>

        <motion.path d="M 25 -10 L 25 -25" stroke="hsl(160 84% 39%)" strokeWidth="3" markerEnd="url(#arrow-head)"
           animate={{ x: [0, 60, 120, 180, 0, 60, 120, 180, 0, 60, 120, 180] }} transition={{ duration: 6, ease: "linear", repeat: Infinity }} />
        <text x="25" y="-35" textAnchor="middle" fontSize="14" fontWeight="800" fill="hsl(160 84% 39%)">Inner c</text>
      </g>
  ...
  return (
    <svg viewBox="0 0 800 450" className="w-full h-full font-sans select-none" style={{ backgroundColor: "hsl(var(--background))" }}>
        ...
        {[0, 1, 2].map(r => (
          <g key={`rm-${r}`}>
            {[0, 1, 2].map(c => {
               ...
               return <rect key={`rmc-${c}`} x={12 + c * 54} y={15 + r * 54} width="48" height="42" rx="6" fill={fillGrad} opacity="0.5" stroke="hsl(0 0% 100%)" strokeWidth="1" />;
            })}
          </g>
        ))}
      </g>
      ...
      <g transform="translate(480, 130)">
        <rect width="80" height="150" rx="12" fill="url(#emerald-grad)" stroke="hsl(0 0% 100%)" strokeWidth="2" filter="url(#soft-shadow)" />
        <rect width="80" height="150" x="90" rx="12" fill="url(#primary-grad)" stroke="hsl(0 0% 100%)" strokeWidth="2" filter="url(#soft-shadow)" />
        <rect width="80" height="150" x="180" rx="12" fill="url(#destructive-grad)" stroke="hsl(0 0% 100%)" strokeWidth="2" filter="url(#soft-shadow)" />

        <text x="40" y="-10" textAnchor="middle" fontSize="14" fontWeight="800" fill="hsl(160 84% 39%)">Row 0</text>
        <text x="130" y="-10" textAnchor="middle" fontSize="14" fontWeight="800" fill="hsl(239 84% 67%)">Row 1</text>
        <text x="220" y="-10" textAnchor="middle" fontSize="14" fontWeight="800" fill="hsl(347 89% 60%)">Row 2</text>
      </g>
      ...
      <text x="400" y="380" textAnchor="middle" fontSize="15" fontWeight="600" fill="hsl(var(--foreground))">
        가로(Row) 덩어리가 메모리에 일렬로 늘어서게 되어, <tspan fill="hsl(239 84% 67%)" fontWeight="800">캐시 효율</tspan>이 극대화됩니다.
      </text>
    </svg>
  );
}
    ...
    <svg viewBox="0 0 800 450" className="w-full h-full font-sans select-none" style={{ backgroundColor: "hsl(var(--background))" }}>
      ...
      <g transform="translate(150, 100)" filter="url(#soft-shadow)">
          ...
          {[0, 1, 2, 3, 4].map(r => (
            <g key={`row-${r}`}>
              {[0, 1, 2, 3, 4].map(c => {
                 ...
                 if(r===2 && c===2) fill = "hsl(239 84% 67%)"; // Center current
                 ...
                 return (
                  <rect key={`cell-${r}-${c}`} x={c * 40} y={r * 40} width="36" height="36" rx="6"
                        fill={fill} stroke={fill === "hsl(239 84% 67%)" ? "none" : "hsl(var(--border))"} strokeWidth="1" />
                 );
              })}
            </g>
          ))}

          <text x="100" y="105" textAnchor="middle" fontSize="16" fontWeight="bold" fill="hsl(0 0% 100%)">현재</text>

          {/* Animated search directions (dx, dy) */}
          <motion.path d="M 100 80 L 100 40" stroke="hsl(160 84% 39%)" strokeWidth="4" markerEnd="url(#arrow-head)"
             initial={{ opacity: 0, scale: 0 }} animate={{ opacity: [0, 1, 0], scale: [0, 1, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 0 }} />
          <motion.path d="M 100 115 L 100 155" stroke="hsl(160 84% 39%)" strokeWidth="4" markerEnd="url(#arrow-head)"
             initial={{ opacity: 0, scale: 0 }} animate={{ opacity: [0, 1, 0], scale: [0, 1, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 0.5 }} />
          <motion.path d="M 80 100 L 40 100" stroke="hsl(160 84% 39%)" strokeWidth="4" markerEnd="url(#arrow-head)"
             initial={{ opacity: 0, scale: 0 }} animate={{ opacity: [0, 1, 0], scale: [0, 1, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 1.0 }} />
          <motion.path d="M 115 100 L 155 100" stroke="hsl(160 84% 39%)" strokeWidth="4" markerEnd="url(#arrow-head)"
        ...
        <g transform="translate(320, 50)" fontSize="16" fontFamily="monospace" fill="hsl(var(--foreground))" fontWeight="600">
          <text x="0" y="20"><tspan fill="hsl(239 84% 67%)">const</tspan> dx = [ <tspan fill="hsl(38 92% 50%)">0</tspan>, <tspan fill="hsl(38 92% 50%)">0</tspan>, <tspan fill="hsl(38 92% 50%)">-1</tspan>, <tspan fill="hsl(38 92% 50%)">1</tspan> ];</text>
          <text x="0" y="45"><tspan fill="hsl(239 84% 67%)">const</tspan> dy = [ <tspan fill="hsl(38 92% 50%)">-1</tspan>, <tspan fill="hsl(38 92% 50%)">1</tspan>, <tspan fill="hsl(38 92% 50%)">0</tspan>, <tspan fill="hsl(38 92% 50%)">0</tspan> ];</text>

          <text x="0" y="85" fontSize="14" fill="hsl(var(--muted-foreground))">// 상, 하, 좌, 우 4방향 탐색 루프</text>
          <text x="0" y="110"><tspan fill="hsl(239 84% 67%)">for</tspan> (<tspan fill="hsl(239 84% 67%)">let</tspan> d=<tspan fill="hsl(38 92% 50%)">0</tspan>; d&lt;<tspan fill="hsl(38 92% 50%)">4</tspan>; d++) {'{'}</text>
          <text x="20" y="135"><tspan fill="hsl(239 84% 67%)">let</tspan> nx = x + dx[d];</text>
          <text x="20" y="160"><tspan fill="hsl(239 84% 67%)">let</tspan> ny = y + dy[d];</text>
          <text x="0" y="185">{'}'}</text>
        </g>
      </g>
      ...
      <text x="400" y="390" textAnchor="middle" fontSize="15" fontWeight="600" fill="hsl(var(--foreground))">
        <tspan fill="hsl(160 84% 39%)" fontWeight="800">델타 배열(dx, dy)</tspan>을 사용하면 if문을 무한정 쓰지 않고 우아하게 사방을 탐색할 수 있습니다.
      </text>
    </svg>
  );
}
