  ...
  return (
    <svg viewBox="0 0 800 450" className="w-full h-full font-sans select-none" style={{ backgroundColor: "hsl(var(--background))" }}>
      ...
      <g transform="translate(100, 160)" filter="url(#soft-shadow)">
        ...
        <text x="80" y="-15" fontSize="16" fontWeight="bold" fill="hsl(239 84% 67%)">메모리 (RAM)</text>
        ...
        <motion.rect x="310" y="45" width="80" height="60" rx="10" fill="hsl(160 84% 39%)" opacity="0.2" stroke="hsl(160 84% 39%)" strokeWidth="4"
          initial={{ opacity: 0, scale: 0.9 }} animate={{ opacity: [0, 1, 0], scale: [0.9, 1.1, 0.9] }} transition={{ duration: 2, repeat: Infinity, delay: 1 }} />

        <motion.path d="M 350 -40 L 350 30" stroke="hsl(160 84% 39%)" strokeWidth="4" strokeLinecap="round" strokeDasharray="6 6"
          initial={{ opacity: 0, y: -20 }} animate={{ opacity: [0, 1, 0], y: [0, 5, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 1 }} />

        <motion.text x="350" y="-50" textAnchor="middle" fontSize="16" fontWeight="900" fill="hsl(160 84% 39%)"
  ...
  return (
    <svg viewBox="0 0 800 450" className="w-full h-full font-sans select-none" style={{ backgroundColor: "hsl(var(--background))" }}>
      ...
      <g transform="translate(100, 100)">
        <text x="0" y="20" fontSize="16" fontWeight="bold" fill="hsl(347 89% 60%)">정적 배열 (크기 초과 시)</text>

        {/* Memory slots */}
        {[0, 1, 2].map((i) => (
          <rect key={`old-${i}`} x={i * 70} y="40" width="60" height="60" rx="8" fill="url(#surface-grad)" stroke={i === 2 ? "hsl(347 89% 60%)" : "hsl(var(--border))"} strokeWidth="2" />
        ))}
        {/* Blocked wall symbol */}
        <rect x="210" y="30" width="10" height="80" rx="4" fill="hsl(347 89% 60%)" />
        <text x="260" y="75" fontSize="14" fontWeight="800" fill="hsl(347 89% 60%)">다른 데이터 공간 (확장 불가)</text>
      </g>
      ...
      <g transform="translate(100, 270)">
        <text x="0" y="20" fontSize="16" fontWeight="bold" fill="hsl(160 84% 39%)">동적 배열 (Re-allocation)</text>

        {/* New larger memory slots */}
        {[0, 1, 2, 3, 4, 5].map((i) => (
          <rect key={`new-${i}`} x={i * 70} y="40" width="60" height="60" rx="8" fill="url(#surface-grad)" stroke={i < 3 ? "hsl(160 84% 39%)" : "hsl(var(--border))"} strokeWidth="2" />
        ))}

        {/* Copy animations */}
        {[0, 1, 2].map((i) => (
          <motion.path key={`arch-${i}`} d={`M ${i * 70 + 30} -70 Q ${i * 70 + 80} -20 ${i * 70 + 30} 40`} stroke="hsl(160 84% 39%)" strokeWidth="3" fill="none"
              initial={{ opacity: 0, pathLength: 0 }}
              animate={{ opacity: [0, 1, 0], pathLength: [0, 1, 1] }}
              transition={{ duration: 2, repeat: Infinity, delay: i * 0.2 }} />
        ))}

        <motion.circle cx="240" cy="70" r="15" fill="hsl(160 84% 39%)" filter="url(#glow)"
  ...
  return (
    <svg viewBox="0 0 800 450" className="w-full h-full font-sans select-none" style={{ backgroundColor: "hsl(var(--background))" }}>
      ...
      <g transform="translate(150, 150)" filter="url(#soft-shadow)">
        <rect width="200" height="150" rx="16" fill="url(#surface-grad)" stroke="hsl(217 91% 60%)" strokeWidth="3" strokeDasharray="10 5" />
        <text x="100" y="30" textAnchor="middle" fontSize="18" fontWeight="800" fill="hsl(217 91% 60%)">수정 가능 (List)</text>

        <rect x="40" y="60" width="120" height="60" rx="8" fill="hsl(217 91% 60%)" opacity="0.1" stroke="hsl(217 91% 60%)" strokeWidth="2" />
        <text x="100" y="96" textAnchor="middle" fontSize="24" fontWeight="900" fill="hsl(var(--foreground))">[ 1, 2, <tspan fill="hsl(347 89% 60%)" textDecoration="line-through">3</tspan>, 4 ]</text>
        <motion.text x="145" y="70" fontSize="24" fontWeight="900" fill="hsl(160 84% 39%)"
      ...
      <g transform="translate(450, 150)" filter="url(#soft-shadow)">
        <rect width="200" height="150" rx="16" fill="url(#surface-grad)" stroke="hsl(160 84% 39%)" strokeWidth="4" />
        <text x="100" y="30" textAnchor="middle" fontSize="18" fontWeight="800" fill="hsl(160 84% 39%)">수정 불가 (Tuple)</text>

        {/* Padlock icon base */}
        <rect x="85" y="-10" width="30" height="20" rx="4" fill="hsl(160 84% 39%)" />
        <path d="M 90 -10 A 10 10 0 0 1 110 -10" fill="none" stroke="hsl(160 84% 39%)" strokeWidth="4" />

        <rect x="40" y="60" width="120" height="60" rx="8" fill="hsl(160 84% 39%)" opacity="0.1" stroke="hsl(160 84% 39%)" strokeWidth="2" />
        <text x="100" y="96" textAnchor="middle" fontSize="24" fontWeight="900" fill="hsl(var(--foreground))">( 1, 2, 3, 4 )</text>

        {/* Reject animation */}
        <motion.line x1="120" y1="50" x2="150" y2="20" stroke="hsl(347 89% 60%)" strokeWidth="4"
           initial={{ opacity: 0 }} animate={{ opacity: [0, 1, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 0.5 }} />
        <motion.line x1="150" y1="50" x2="120" y2="20" stroke="hsl(347 89% 60%)" strokeWidth="4"
           initial={{ opacity: 0 }} animate={{ opacity: [0, 1, 0] }} transition={{ duration: 2, repeat: Infinity, delay: 0.5 }} />
      </g>

      <rect x="150" y="350" width="500" height="50" rx="25" fill="hsl(var(--muted))" opacity="0.6" stroke="hsl(var(--border))" strokeWidth="1" />
      <text x="400" y="380" textAnchor="middle" fontSize="15" fontWeight="600" fill="hsl(var(--foreground))">
        데이터가 변조되지 않음을 <tspan fill="hsl(160 84% 39%)" fontWeight="800">시스템 레벨에서 보장</tspan>하여 동시성 오류를 예방합니다.
      </text>
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 800 450" className="w-full h-full font-sans select-none" style={{ backgroundColor: "hsl(var(--background))" }}>
      ...
      <g transform="translate(100, 100)" filter="url(#soft-shadow)">
        ...
        <text x="300" y="32" textAnchor="middle" fontSize="18" fontWeight="900" fill="hsl(239 84% 67%)">배열 (Array)</text>
        <text x="500" y="32" textAnchor="middle" fontSize="18" fontWeight="900" fill="hsl(160 84% 39%)">연결 리스트 (Linked List)</text>
        ...
        <g transform="translate(230, 65)">
           <rect width="140" height="30" rx="6" fill="hsl(160 84% 39%)" opacity="0.2" stroke="hsl(160 84% 39%)" strokeWidth="1" />
           <text x="70" y="20" textAnchor="middle" fontSize="15" fontWeight="900" fill="hsl(160 84% 39%)">O(1) (매우빠름)</text>
        </g>
        <text x="500" y="82" textAnchor="middle" fontSize="15" fontWeight="700" fill="hsl(347 89% 60%)">O(N) (탐색필요)</text>
        ...
        <text x="300" y="134" textAnchor="middle" fontSize="15" fontWeight="700" fill="hsl(347 89% 60%)">O(N)</text>
        <text x="500" y="134" textAnchor="middle" fontSize="15" fontWeight="700" fill="hsl(347 89% 60%)">O(N)</text>
        ...
        <text x="300" y="186" textAnchor="middle" fontSize="15" fontWeight="700" fill="hsl(160 84% 39%)">O(1)</text>
        <text x="500" y="186" textAnchor="middle" fontSize="15" fontWeight="700" fill="hsl(160 84% 39%)">O(1)</text>

        {/* Row 4: Insert Middle */}
        <text x="100" y="238" textAnchor="middle" fontSize="15" fontWeight="700" fill="hsl(var(--foreground))">중간 삽입/삭제</text>
        <text x="300" y="238" textAnchor="middle" fontSize="15" fontWeight="700" fill="hsl(347 89% 60%)">O(N) (이동발생)</text>
        <g transform="translate(430, 221)">
           <rect width="140" height="30" rx="6" fill="hsl(160 84% 39%)" opacity="0.2" stroke="hsl(160 84% 39%)" strokeWidth="1" />
           <text x="70" y="20" textAnchor="middle" fontSize="15" fontWeight="900" fill="hsl(160 84% 39%)">O(1) (참조만변경)</text>
        </g>
      </g>
