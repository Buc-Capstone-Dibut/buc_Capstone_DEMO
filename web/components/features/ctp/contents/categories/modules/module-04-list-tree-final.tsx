...
const TREE_MODULES = createInteractiveTemplateModules([
  {
    ...
    story: {
      ...
      definition:
        "**트리(Tree)** 는 부모-자식 관계로 이어진 노드들의 비선형 자료구조입니다. **루트(root)** 는 부모가 없는 최상위 노드로 트리에 단 한 개만 존재합니다. **리프(leaf)** 는 자식이 없는 말단 노드입니다. **깊이(depth)** 는 루트에서 특정 노드까지의 간선 수, **높이(height)** 는 가장 깊은 리프까지의 간선 수입니다. 깊이는 노드별 속성, 높이는 트리 전체의 속성입니다.",
      analogy:
...
const FINAL_CHALLENGE_MODULES = createInteractiveTemplateModules([
  ...
  },
  {
    ...
    story: {
      ...
      definition:
        "**FC-4** 는 제한 시간 안에 문제를 읽고 풀이를 설계·구현하는 미니 코딩 테스트입니다. 평가 영역은 문제 정독, 전략 후보 비교, 자료구조 선택, 단일 패스 구현, 엣지 케이스 점검입니다. 통과 기준은 정답성, 시간 내 제출, 핵심 설계 의도 설명 세 가지이고, 구현 직전에 \"무엇을 어떻게 쓰겠다\"를 먼저 명문화하는 습관을 검증합니다.",
      analogy:
    ...
    features: [
      {
        title: "타이머 기반 흐름 통제",
        description:
          "정독·설계·구현·점검 4단계에 각각 시간을 배분합니다. 처음부터 구현으로 뛰어들지 않고 설계에 일정 시간을 확보하는 습관을 익힙니다.",
