...
/**
 * 웹슬라이드 포맷 편집기의 좌측 페이지 사이드바.
 *
 * - 페이지 카드 리스트 (드래그로 순서 변경 — @dnd-kit/sortable)
 * - 현재 페이지 강조
 * - 표시/숨김 토글 (Eye/EyeOff)
 * - 삭제 (휴지통, 확인 alert)
 * - "+ 슬라이드 추가" — 페이지 타입 선택 드롭다운
 */
...
type PortfolioSitePagesSidebarProps = {
  pages: PortfolioSitePage[];
  activePageId: string | null;
  onSelectPage: (pageId: string) => void;
  onReorderPages: (nextOrder: string[]) => void;
...
export function PortfolioSitePagesSidebar({
  ...
  onAddPage,
  disabled,
}: PortfolioSitePagesSidebarProps) {
  ...
  return (
    ...
    >
      <div className="flex h-12 shrink-0 items-center justify-between px-4 border-b border-[#d8e4d0]/60">
        ...
        <div className="relative">
          <button
            ...
            disabled={disabled}
            className="flex h-7 w-7 items-center justify-center rounded-md border border-primary/30 bg-primary/8 text-primary transition hover:bg-primary/14 disabled:cursor-not-allowed disabled:opacity-40"
            aria-label="슬라이드 추가"
          >
