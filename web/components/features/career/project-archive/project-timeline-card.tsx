...
interface ProjectTimelineCardProps {
  ...
  onCardClick: (project: ProjectInput) => void;
      ...
      <div className="w-1/2 pr-10 md:pr-14 flex justify-end">
        <div
          ...
          className={cn(
            ...
            portfolioMode &&
              "border-2 border-primary/30 bg-primary/5 shadow-md hover:border-primary dark:bg-primary/10",
            isSelected &&
              selectionMode &&
        ...
        >
          ...
          {!selectionMode && !isActive && project.id && (
            <button
              type="button"
              onClick={(event) => {
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {project.techStack.slice(0, 6).map((tech) => {
                ...
                return (
                  <span
                    key={tech}
                    className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-200/70 bg-white px-2.5 text-xs font-bold text-slate-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"
                  >
                    {logo?.url ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={logo.url}
                        alt={logo.alt || `${tech} 로고`}
