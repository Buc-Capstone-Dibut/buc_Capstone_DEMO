...
export function SoftPastelCardRenderer({ document, className }: RendererProps) {
  ...
  if (!page) {
    return (
      <div className={cn("flex min-h-screen items-center justify-center", className)} style={{ background: BG_GRADIENT, color: MUTED }}>
        슬라이드가 없습니다.
      </div>
    );
  }
  ...
  return (
    <section
      ...
      style={{ background: BG_GRADIENT, color: TEXT, fontFamily: "Pretendard, Inter, system-ui, sans-serif" }}
    >
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col px-4 py-4 sm:px-6">
        <header className="flex h-12 items-center justify-between rounded-full bg-white/60 px-5 py-2 shadow-sm backdrop-blur-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={index === 0}
              className="flex h-7 w-7 items-center justify-center rounded-full bg-white shadow-sm transition hover:shadow-md disabled:cursor-not-allowed disabled:opacity-40"
              aria-label="이전"
            >
              <ChevronLeft className="h-3.5 w-3.5" style={{ color: PRIMARY }} />
            </button>
            <button
              type="button"
              onClick={() => setIndex((i) => Math.min(pages.length - 1, i + 1))}
              disabled={index >= pages.length - 1}
              className="flex h-7 w-7 items-center justify-center rounded-full shadow-sm transition hover:shadow-md disabled:cursor-not-allowed disabled:opacity-40"
        ...
        <div className="flex h-12 items-center gap-1.5 overflow-x-auto pt-3">
          {pages.map((p, i) => (
            <button
              key={p.id}
              type="button"
              onClick={() => setIndex(i)}
              className={cn(
                "flex h-8 shrink-0 items-center gap-1.5 rounded-full px-3 text-[11px] font-bold transition",
              )}
              style={{
                backgroundColor: i === index ? PRIMARY : "white",
                color: i === index ? "white" : TEXT,
                boxShadow: i === index ? SOFT_SHADOW : "0 2px 4px rgba(0,0,0,0.04)",
              }}
            >
