...
class NonverbalPerAnswer(BaseModel):
    index: int = 0
    comment: str = ""
...
class NonverbalSummary(BaseModel):
    overall: str = ""
    perAnswer: list[NonverbalPerAnswer] = Field(default_factory=list)
...
SJT 시나리오는 {company}에서 실제로 발생할 법한 상황으로 구성하세요."""
    ...
    def _default_question_plan(self, context: dict[str, Any]) -> list[QuestionPlanItem]:
        ...
        if track == "role":
            return [
                QuestionPlanItem(
                    slot=1,
                    phase="introduction",
                    intent="직무와 연결되는 대표 프로젝트 맥락 확인",
                    questionBlueprint=f"{role}와 가장 가까운 프로젝트를 하나 골라 문제, 본인 역할, 구현 범위를 설명하게 유도",
                    targetCompetency="직무 경험 연결성",
                    evaluationSignals=["프로젝트 맥락", "담당 범위", "직무 연관성"],
                ),
                QuestionPlanItem(
                    slot=2,
                    phase="technical",
                    intent="직무 핵심 역량과 실제 구현 방식 검증",
                    questionBlueprint=f"{tech_stack} 맥락에서 직접 구현하거나 판단한 부분을 구체적으로 설명하게 유도",
                    targetCompetency="직무 핵심 역량",
                    evaluationSignals=["구현 디테일", "기술 이해도", "본인 기여"],
                ),
                QuestionPlanItem(
                    slot=3,
                    phase="technical",
                    intent="문제 발생 시 원인 추적과 해결 순서 확인",
                    questionBlueprint="개발 중 문제가 생겼을 때 재현, 원인 추적, 수정 확인을 어떤 순서로 진행했는지 질문",
                    targetCompetency="문제 해결력",
                    evaluationSignals=["원인 분석", "해결 순서", "검증 방식"],
                ),
                QuestionPlanItem(
                    slot=4,
                    phase="situational",
                    intent="협업 방식과 팀 내 역할 확인",
                    questionBlueprint="팀 프로젝트에서 의견 조율이나 역할 분담이 필요했던 상황을 어떻게 풀었는지 질문",
                    targetCompetency="협업 역량",
                    evaluationSignals=["커뮤니케이션", "역할 인식", "조율 방식"],
                ),
                QuestionPlanItem(
                    slot=5,
                    phase="closing",
                    intent="직무 성장 방향과 보완 계획 확인",
                    questionBlueprint=f"{role}로 성장하기 위해 최근 보완 중인 부분이나 다음 프로젝트에서 더 깊게 가져가고 싶은 부분을 질문",
                    targetCompetency="성장 가능성",
                    evaluationSignals=["자기 인식", "학습 방향", "현실성"],
                ),
            ]
        ...
        return [
            QuestionPlanItem(
                slot=1,
                phase="introduction",
                intent="1분 자기소개와 지원 동기 확인",
                questionBlueprint=f"{role} 포지션에 지원한 이유와 이력서에서 공고와 가장 맞닿은 경험을 1분 자기소개로 설명하게 유도",
                targetCompetency="지원 동기/직무 적합성",
                evaluationSignals=["지원 동기의 구체성", "경험 연결성", "직무 이해도"],
            ),
            QuestionPlanItem(
                slot=2,
                phase="experience",
                intent="공고 요구사항과 이력서/포트폴리오 연결 검증",
                questionBlueprint=f"{tech_stack} 또는 주요 업무와 연결되는 프로젝트에서 본인이 실제 맡은 범위를 설명하게 유도",
                targetCompetency="공고-경험 매칭",
                evaluationSignals=["담당 범위", "직접 기여", "공고 요구 연결"],
            ),
            QuestionPlanItem(
                slot=3,
                phase="technical",
                intent="기술적 어려움과 해결 과정 검증",
                questionBlueprint=f"{tech_stack} 중 실제로 어려웠던 기술 문제를 하나 고르고 원인 파악과 해결 과정을 설명하게 유도",
                targetCompetency="기술적 판단력",
                evaluationSignals=["문제 정의", "해결 접근", "결과 확인"],
            ),
            QuestionPlanItem(
                slot=4,
                phase="situational",
                intent="협업 성향과 실제 면접형 인성 질문",
                questionBlueprint="팀으로 일할 때 본인의 협업 방식, 갈등 조율 경험, 일정과 품질이 부딪힌 상황의 대응을 질문",
                targetCompetency="협업/태도",
                evaluationSignals=["협업 방식", "조율 태도", "책임감"],
            ),
            QuestionPlanItem(
                slot=5,
                phase="closing",
                intent="마지막 어필과 합류 후 기여 가능성 확인",
                questionBlueprint=f"{role}에 합류한다면 바로 기여할 수 있는 부분이나 마지막으로 강조하고 싶은 강점을 질문",
                targetCompetency="마무리 적합성",
                evaluationSignals=["강점 명확성", "회사/직무 연결", "현실성"],
            ),
        ]
    ...
    def build_interview_plan(self, context: dict[str, Any], retries: int = 1) -> list[dict[str, Any]]:
        ...
        if track == "role":
            slot_guidance = """
슬롯 매핑:
- slot1: introduction (직무와 연결되는 대표 프로젝트 맥락)
- slot2: situational (직무 핵심 역량과 실제 구현 방식)
- slot3: situational (문제 추적/디버깅/검증 방식)
- slot4: technical (협업 방식과 팀 내 역할)
- slot5: closing (성장 방향/보완 계획)

질문 작성 원칙:
- 특정 회사 지원 동기보다 해당 직무 자체의 경험과 준비도를 묻습니다
- 프로젝트에서 맡은 범위, 기술적으로 어려웠던 점, 테스트/검증 방식, 팀 협업 경험을 자연스럽게 포함합니다
- 지표/병목/트레이드오프 같은 단어를 반복하지 말고 답변 단서가 있을 때만 사용합니다
"""
        else:
            slot_guidance = """
슬롯 매핑:
- slot1: introduction (1분 자기소개/지원동기)
- slot2: situational (공고 요구사항과 이력서·포트폴리오 연결)
- slot3: situational (기술적 어려움과 해결 과정)
- slot4: technical (협업 성향/인성 질문/팀 내 역할)
- slot5: closing (마지막 어필/합류 후 기여 가능성)

질문 작성 원칙:
- 실제 지원 면접처럼 회사와 직무를 선택한 이유, 이 공고와 맞닿은 경험, 이력서·포트폴리오 검증을 포함합니다
- 인성 질문은 협업 방식, 갈등 조율, 일정과 품질이 부딪힌 상황처럼 직무 맥락 안에서 묻습니다
- 지표/병목/트레이드오프 같은 단어를 반복하지 말고 답변 단서가 있을 때만 사용합니다
"""
...
"""
    ...
    def _fallback_question(
        ...
        job_data = context.get("jobData", {}) or {}
        role = job_data.get("role", "해당 직무")
        if infer_interview_track(job_data) == "role":
            fallback_by_slot = {
                1: f"{role}와 가장 가까운 프로젝트 하나를 골라 문제, 본인 역할, 구현 범위를 설명해 주세요.",
                2: f"{role}에서 중요하다고 보는 역량 중 본인이 가장 자신 있는 부분을 실제 경험과 함께 말씀해 주세요.",
                3: "개발 중 문제가 생겼을 때 원인을 어떤 순서로 좁히고 해결 여부를 확인하는 편인지 설명해 주세요.",
                4: "팀 프로젝트에서 역할 분담이나 의견 조율이 필요했던 순간을 어떻게 해결했는지 말씀해 주세요.",
                5: f"{role}로 성장하기 위해 최근 보완하고 있는 부분을 말씀해 주세요.",
            }
        else:
            fallback_by_slot = {
                1: f"{role} 포지션에 지원한 이유와 본인의 핵심 강점을 한 가지 사례와 함께 말씀해 주세요.",
                2: "이 공고의 주요 업무와 가장 맞닿은 이력서나 포트폴리오 경험을 하나 골라 본인 역할을 설명해 주세요.",
                3: "그 경험에서 가장 어려웠던 기술 문제를 하나 꼽고, 원인 파악과 해결 과정을 설명해 주세요.",
                4: "팀으로 일할 때 본인의 협업 방식이 잘 드러났던 상황을 하나 말씀해 주세요.",
                5: f"{role}에 합류한다면 바로 기여할 수 있다고 보는 강점 한 가지를 마지막으로 말씀해 주세요.",
            }
    ...
    def generate_next_question_structured(
        ...
        if question_index <= len(normalized_plan):
            ...
            plan_item = QuestionPlanItem(
                ...
                intent="직전 답변의 구체 상황과 판단 과정을 깊게 검증",
                questionBlueprint="직전 답변에서 본인 행동, 판단 과정, 결과 확인 방식을 자연스럽게 확인하는 꼬리질문",
                targetCompetency="문제 해결력",
                evaluationSignals=["구체 상황", "본인 행동", "결과 확인 방식"],
            )
...
6) 지표, 병목, 트레이드오프, 근거 같은 단어를 반복적으로 쓰지 말고, 직전 답변에 직접 단서가 있을 때만 사용
"""
...
"""
            response = self._generate_report(prompt)
            payload = _extract_json(self._response_text(response))
            validated = NonverbalSummary.model_validate(payload)
            return validated.model_dump()
        except Exception as exc:  # noqa: BLE001
            logger.warning("[nonverbal] analyze_nonverbal failed; returning empty default: %s", exc)
